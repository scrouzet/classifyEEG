...
res.integrat_win   = 50;         % size of the past time-window to integrate from, in ms
res.times          = EEG.times;
...
for t = 1:res.n_time % loop over time-points
    ...
    Xo = squeeze(X(:,t:t+(res.intwin-1),:));
    Xo = reshape(Xo,[size(Xo,1)*size(Xo,2) size(Xo,3)])';
...
save('example_results_integration.mat','res','-v7.3');