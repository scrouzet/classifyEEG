...
function M = dectopo(data, chan_locs, deco_timecourse, varargin)
...
if plottimes_set == 0
  ...
  if nargin< 5 | isempty(g.plottimes)
...
set(axdata,'Color',BACKCOLOR);
pl=plot(x,deco_timecourse);    % plot the data
...
yl=ylabel('Decoding performance');
...
for t=1:ntopos % dfraw vertical lines through the data at topoplot frames
 if length(g.plotchans)>1 | g.voffsets(t)
  l1 = plot([g.plottimes(t) g.plottimes(t)],...
       [ymin ymax],'w'); % white underline behind
  set(l1,'linewidth',2);
  l1 = plot([g.plottimes(t) g.plottimes(t)],...
       [ymin ymax],'Color', [0.7 0.7 0.7]); % blue line
