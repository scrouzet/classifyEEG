function[h_plot, h_fill] = sliding_plot(mytimes, dec_acc, mycolor, dec_acc_ci)
...
if nargin > 3
    % Plot CI
    h_fill = fill([mytimes fliplr(mytimes) ], [ dec_acc_ci(1,:) fliplr(dec_acc_ci(2,:)) ],...
        mycolor, 'EdgeColor', 'none', 'FaceAlpha', 0.3); hold on
end

if ~isempty(dec_acc)
    % Plot average across CV
    h_plot = plot(mytimes, dec_acc, 'Color', mycolor); hold on
end
